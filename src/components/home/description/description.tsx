"use client"

import Image from "next/image"
import { useState } from "react"

const PLACEHOLDER_IMAGE = 'data:image/jpeg;base64,';

export const Description = () => {
  const [hasBorder, setBorder] = useState(false);
  
  const handleClick = () => setBorder(!hasBorder);

  return (
    <div className="grid grid-cols-2 gap-x-9 max-w-1000 mx-auto my-24 px-80 py-8 rounded-xl">
      <div className="relative">
        <Image
          className="object-cover rounded-lg shadow-custom"
          src="/images/description.jpeg"
          alt="products marketplace"
          width={500}
          height={300}
          placeholder="blur"
          blurDataURL={PLACEHOLDER_IMAGE}
        />
      </div>
      <div>
        <h2 className="text-2xl font-semibold text-secondary my-0">Bring the future today</h2>
        <p className="text-lg leading-relaxed text-text-color py-4">Future World: Your Gateway to Tomorrow's Tech! Dive into a world of cutting-edge gadgets and gear. Stay ahead of the curve and redefine your digital lifestyle with us.</p>
      </div>
    </div>
  )
}